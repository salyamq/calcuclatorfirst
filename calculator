...
def calculate():
    ...
    try:
        ...
        messagebox.showinfo('Внимание', 'Вы уверены?        @salyamq')
        calc.insert(0, 0)
    except ZeroDivisionError:
        messagebox.showinfo('Внимание', 'На ноль делить нельзя!       @salyamq')
        calc.insert(0, 0)
...
def make_digit_button(digit):
    return tk.Button(text=digit, font=("Arrial", 13), command= lambda: add_digit(digit))

def make_operation_button(operation):
    return tk.Button(text=operation, font=("Arrial", 13), bg="#A9A9A9",
...
calc = tk.Entry(win, justify=tk.RIGHT, font = ('Arrial', 15), width=15, bg = "black", fg = "white")
calc.insert(0, "0")
